...
if strcmp(host,'DESKTOP-LBOSF73')
    table_path = 'D:\Dropbox\Applications\Overleaf\Risk Sharing in a Dual Labor Market\tables\';
end
if ( strcmp(host, 'DESKTOP-NSR4FR4') || strcmp(host, 'PC-SALON') || strcmp(host, 'DESKTOP-OV023AQ') )
    table_path = ['C:\Users\',getenv('username'),'\Dropbox\Applications\Overleaf\Risk Sharing in a Dual Labor Market\tables\'];
end
clearvars host
...
notes = ['\t\\caption*{\\footnotesize Notes: parameter values in the benchmark U.S.\\ calibrated economy.', ...
    'The calibration target for the exogenous separation probability $\\delta$ is the ratio between the EU probability of jobs ', ...
    'with less than one year of tenure and the average of the same probability for jobs with one-to-fifteen years of tenure ', ...
    '(estimated using 1996-2010 Job Tenure Supplement data of the CPS). The other preset parameters are standard. The calibration ', ...
    'targets for the internal parameters are: an unemployment rate equal to 5.6\\%%, an aggregate EU probability of 2.00\\%% (\\cite{jung_kuhn:2014:EJ}), ', ...
    'and an unemployment-benefit replacement ratio (unemployment income to average wage) equals to 0.40 (\\cite{shimer:2005:AER}). The log ', ...
    'match-quality average $\\mu_x = -\\sigma_x^2/2$: the unconditional (i.e., sampling) match quality has mean normalized to one. The value ', ...
    'of $\\kappa$ is set to be consistent with the normalization $\\theta=1$ (\\cite{shimer:2005:AER}). The model fit to the calibration targets ', ...
    'is shown in Table \\ref{tab:targets}, Panel A.}\n' ];
...
for j = 1:L
    ...
    % average profits
    pi(j) = agg_stat{j}.cJ;
    pi_new(j) = (1-bbeta)*eql{j}.Jnew;

    % aggregate hiring/firing costs
    HC(j) = (agg_stat{j}.theta*agg_stat{j}.U)*kkappa;
    FC(j) = (1-agg_stat{j}.U)*(1-agg_stat{j}.T)*(agg_stat{j}.PU-ddelta)/(1-ddelta)*F(j);
...
tab = ['\\begin{table}[!h]' '\n' ...
    ...
    '\\hspace{5pt} Hiring profits $\\overline{\\pi}_0$ & '    s(pi_new)     '\\\\' '\n' ...
    '\\hspace{5pt} Hiring costs/output & '                    s(HC)         '\\\\' '\n' ...
    '\\hspace{5pt} Average log wage & '                       s(Wmn)        '\\\\' '\n' ...
    '\\hspace{5pt} Hiring log wage & '     s(Wnew)       '\\\\' '\n' ...
    '\\addlinespace' '\n' ...
